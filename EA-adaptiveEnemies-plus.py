...
class EvoMan:
    ...
    def tournament_selection_children(self, candidate_indices, population, remember_fitness):
        # Generate k unique random indices
        random_indices = np.random.choice(candidate_indices, self.k_tournament, replace=False)
        ...
        fitness, health_gain, time_game, player_life, enemy_life = self.simulation(best_individual[:-1])
        fitness = np.array(self.fitness_function(player_life, enemy_life, time_game))
        health_gain = np.array(health_gain)
        #print(health_gain)
        time_game = np.array(time_game)
        
        return best_individual_index, np.max(fitness), health_gain, time_game, remember_fitness
    ...
    def selection(self, population):        
        ...
        for i in range(self.n_pop-self.n_elitism):
            winner_index, fitness[i], health_gain[i], time_game[i], remember_fitness = self.tournament_selection_children(candidate_indices, population, remember_fitness)
    ...
    def train(self):
        ...
        with open(results_file_path, mode='w', newline='') as file:
            ...
            for gen in range(self.gens):
                ...
                # if self.n_elitism > 0:
                #     selected_indices = [i for i in range(len(population)) if i not in parents_survivors_indices]
                #     selected_population = np.array([population[i] for i in selected_indices])
                #     pop_without_elite = np.array([population[i] if i not in parents_survivors_indices for i in len(population)])
                ...
                if self.n_elitism > 0:
                    ...
                    population, fitness, health_gain, time_game = self.selection(pop_before_selection) 
