...
class player_controller(Controller):
	...
	def control(self, inputs, controller):
		...
		# Replace sigmoid_activation with relu_activation in the control method
		if self.n_hidden[0] > 0:
			# Outputs activation first layer.
			output1 = np.maximum(0,inputs.dot(self.weights1) + self.bias1)

			# Outputting activated second layer. Each entry in the output is an action
			output = np.maximum(0,output1.dot(self.weights2) + self.bias2)[0]
		else:
			bias = controller[:5].reshape(1, 5)
			weights = controller[5:].reshape((len(inputs), 5))

			output = np.maximum(inputs.dot(weights) + bias)[0]
...
class enemy_controller(Controller):
	...
	def control(self, inputs,controller):
		...
		if self.n_hidden[0]>0:
			...
			# Outputs activation first layer.
			output1 = np.maximum(0, inputs.dot(weights1) + bias1)
			...
			# Outputting activated second layer. Each entry in the output is an action
			output = np.maximum(0,output1.dot(weights2)+ bias2)[0]
		else:
			bias = controller[:5].reshape(1, 5)
			weights = controller[5:].reshape((len(inputs), 5))

			output = np.maximum(0,inputs.dot(weights) + bias)[0]
