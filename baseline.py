...
class EvoMan:
    ...
    def generate_baseline(self):
        population = np.array([self.initialize_individual() for _ in range(8000)])
        
        # Lists to hold the fitnesses and health gains of each individual
        all_fitnesses = []
        all_health_gains = []
        all_enemies_beaten = []

        # For each individual, run the simulation against all 8 enemies and store the results
        for individual in population:
            _, health_gains, times, player_lifes, enemy_lifes = self.simulation(individual[:-1])
            fitness = self.fitness_function(player_lifes, enemy_lifes, times)
            enemies_beaten = sum([gain > 0 for gain in health_gains])
            
            all_fitnesses.append(fitness)
            all_health_gains.append(health_gains)
            all_enemies_beaten.append(enemies_beaten)

        # Save the results to a CSV file
        baseline_file_path = os.path.join(self.experiment_dir, "baseline.csv")
        with open(baseline_file_path, mode='w', newline='') as file:
            writer = csv.writer(file)
            writer.writerow(["Individual", "Fitness", 
                            "HealthGain1", "HealthGain2", "HealthGain3", "HealthGain4", "HealthGain5", "HealthGain6", "HealthGain7", "HealthGain8",
                            "EnemiesBeaten"])
            for i, (f, h) in enumerate(zip(all_fitnesses, all_health_gains)):
                writer.writerow([i+1, f] + list(h) + [all_enemies_beaten[i]])
    ...
    def test(self):
        ...
        if os.path.exists(best_individual_path):
            ...
            'average_health_gain': avg_health_gain,
            'total_health_gain': np.sum(health_gain),
...
if __name__ == "__main__":
        ...
        parser.add_argument("--mode", type=str, default="train", help="Mode: train, test or baseline")
